...
TARGET_IMAGE_NAME="10.210.60.130:5000/zubin-ui:16461"
...
docker run --rm -it \
  ...
  -v /root/cosign/data:/cosign/data \
  cosign sh -c "\
    cosign clean ${TARGET_IMAGE_NAME} -f && \
    cosign load --dir /cosign/data/${IMAGE_NAME} ${TARGET_IMAGE_NAME} && \
    cosign verify \
